...
### 05/28/21 add filtering by % of hemoglobin gene content
...
runQC <- function(inputMatrix=NULL,                   # args for getData
		  ...
		  hemoCut=NULL,                       # args for qcFilter
...
qcFilter <- function(sceset,species="human", addSpikeGenes="",
    ...
    if(checkManCuts && is.null(countCut) && is.null(geneCut)#hiGeneCut can be NULL
       && is.null(mitoCut) && is.null(hemoCut)){
        stop("Please, provide cut values for checking")
    }
    if((checkManCuts || doCuts) && (manFiltFlag || compareFiltFlag)
       && is.null(countCut) && is.null(geneCut) && is.null(mitoCut) && is.null(hemoCut)){
        stop("Please, provide cut values")
    }
    geneID <- match.arg(geneID)
...
###
    ...
    ##
    is.mito <- rownames(sceset) %in% mtList
    message(">>>Found ",length(which(is.mito))," mitochondrial genes")

    ## hemoglobin genes (lists from going through 10x features files and
    ## cross-checking with google)
    if(geneID=="ensembl"){
        if(species=="mouse"){
	  hemoList <- c("ENSMUSG00000073940","ENSMUSG00000052305","ENSMUSG00000078621",
	                "ENSMUSG00000052217","ENSMUSG00000052187","ENSMUSG00000055609",
			"ENSMUSG00000069919","ENSMUSG00000073063","ENSMUSG00000069917",
			"ENSMUSG00000020295")
	} else if(species=="human"){
	  hemoList <- c("ENSG00000244734","ENSG00000223609","ENSG00000206177",
	                "ENSG00000206172","ENSG00000188536","ENSG00000086506")
	} else {stop("Unknown species")}
    }  else if(geneID=="symbol"){
       if(species=="mouse"){
          hemoList <- c("Hbb-bt","Hbb-bs","Hbb-bh2","Hbb-bh1","Hbb-y",
	                "Hba-x","Hba-a1","Hbq1b","Hba-a2","Hbq1a")
        } else if(species=="human"){
          hemoList <- c("HBB","HBD","HBM","HBA1","HBA2","HBQ1")
        } else {stop("Unknown species")}
    } else {stop("Unknown geneID")}
    ##
    is.hemo <- rownames(sceset) %in% hemoList
    message(">>>Found ",length(which(is.hemo))," hemoglobin genes")

### Calculating QC metrics using scater package (lots of stuff to use later)

    sceset <- addPerCellQC(sceset,subsets = list(MT = is.mito, HEMO = is.hemo))
#    return(sceset)
    gc()
############# TEMPORARY
    ...
    if(doCuts || compareFiltFlag){
        if(manFiltFlag || compareFiltFlag){
            ...
            sceset <- manualFilter(sset=sceset,tablesOut=tablesOut,
                                   countCut=countCut,geneCut=geneCut,
				   mitoCut=mitoCut,hiGeneCut=hiGeneCut,
				   hemoCut=hemoCut,outPlotDir=outPlotDir)
        }
...
###                   selected_variables=c("pct_counts_MT","total_features_by####_counts","total_counts"))
    sset <- runColDataPCA(x = sset,
			  variables=c("subsets_MT_percent","subsets_HEMO_percent","detected","sum"),
			  outliers = TRUE)
...
manualFilter <- function(sset,tablesOut=TRUE,
    ...
    filter_by_total_counts <- (sset$sum > countCut)
    filter_by_gene_counts <- (sset$detected > geneCut) & (sset$detected < hiGeneCut)
    filter_by_MT <- (sset$subsets_MT_percent < mitoCut)
    if(is.null(hemoCut)){
      filter_by_HEMO <- TRUE
    } else {
      filter_by_HEMO <- (sset$subsets_HEMO_percent < hemoCut)
    }
	...
	cat("table(filter_by_HEMO)\n")
        print(table(filter_by_HEMO))
        cat("table(sset$use)\n")
...
### library size
    ...
    png(fileOut)
    hist(sset$sum,breaks=1000,main="Library size",
...
### detected genes
    ...
    png(fileOut)
    hist(sset$detected,breaks=1000,main="Detected genes",
...
### mitochondrial genes
    ...
    par(mfrow=c(1,2))
    ## 1d part
    hist(sset$subsets_MT_percent,breaks=1000,main='',xlab="Mitochondrial read %",
         cex.lab=1.5,cex.axis=1.2)
    if(plotCuts && !is.null(mitoCut)) abline(v = mitoCut, col = "red")
    ## 2d part
    if(plotCuts && !is.null(mitoCut)){
        hcols<-ifelse(sset$subsets_MT_percent>mitoCut,"red","black")
    } else {
        hcols <- "black"
    }
    plot(sset$detected,sset$subsets_MT_percent,col=hcols,
    ...
    dev.off()
###
### hemoglobin genes genes
    if(plotCuts && !is.null(hemoCut)){
      fileOut <- paste0(runDir,"/hemoGenesHist_run_",run,"_cut",hemoCut,"_",today,".png")
    } else {
        fileOut <- paste0(runDir,"/hemoGenesHist_run_",run,"_",today,".png")
    }
    png(fileOut)
    par(mfrow=c(1,2))
    ## 1d part
    hist(sset$subsets_HEMO_percent,breaks=1000,main='',xlab="Hemoglobin read %",
         cex.lab=1.5,cex.axis=1.2)
    if(plotCuts && !is.null(hemoCut)) abline(v = hemoCut, col = "red")
    ## 2d part
    if(plotCuts && !is.null(hemoCut)){
        hcols<-ifelse(sset$subsets_HEMO_percent>hemoCut,"red","black")
    } else {
        hcols <- "black"
    }
    plot(sset$detected,sset$subsets_HEMO_percent,col=hcols,
         xlab="Detected genes",ylab="Hemoglobin read %",
         cex.axis=1.2,cex.lab=1.5)
    mtext("Hemoglobin genes", outer = TRUE, cex = 2.2,
          side = 3, line = -3)
    dev.off()
}
